...
def download_mp4(URL: str, PATH: str) -> None:
    try:
        ...
        if FLAG == 'file':
            ...
            call(['yt-dlp', '-f', 'bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best', '-o',
...
def download_mp3(URL: str, PATH: str) -> None:
    try:
        ...
        if FLAG == 'file':
            ...
            call(['yt-dlp', '-x', '--audio-format', 'mp3', '-o',
...
install_packs = tk.IntVar()
mp3 = tk.IntVar()
mp4 = tk.IntVar()

## Checkbox Widget - Abhängigkeiten ##
pack_button = tk.Checkbutton(root, text="Abhängigkeiten installieren", variable=install_packs)
...
## Checkbox Widget - MP3 ##
mp3_button = tk.Checkbutton(root, text="MP3 Download", variable=mp3)
mp3_button.pack(padx=5, pady=5)

## Checkbox Widget - MP4 ##
mp4_button = tk.Checkbutton(root, text="MP4 Download", variable=mp4)
...
def get_path() -> None:
    ...
    loc_path = tk.filedialog.askdirectory()
    root2.iconify()
    root2.deiconify()
    root2.destroy()
...
output_button = tk.Button(root, text="Durchsuchen..", command=get_path)
output_button.pack()

## Button Widget - Download starten ##
def start_download():
    def download_thread():
...
download_button = tk.Button(root, text="Download starten", command=start_download)
download_button.pack(padx=15, pady=15)

## Text Widget - Konsolenausgabe ##
console_text = tk.Text(root, height=10, width=45)
